...
uint8_t* BMP_565_Create(uint32_t width, uint32_t height)
{
    ...
    if(width & 0x00000001)
        bytes_per_row = (width + 1) << 1;
    else
        bytes_per_row = width << 1;
    /*
    uint32_t bytes_per_row = width * 2;
    bytes_per_row += ((bytes_per_row & 0x00000003) ? 4 - bytes_per_row % 4 : 0 );
    */
...
void BMP_565_SetPixelRGB(uint8_t* pbmp, uint32_t x, uint32_t y, uint8_t r, uint8_t g, uint8_t b)
{
    ...
    */
    ...
    uint16_t col = ((uint16_t)(r >> 3) << 11)
                 | ((uint16_t)(g >> 2) <<  5)
                 |  (uint16_t)(b >> 3);
    /*
    uint16_t col = 0x0000;
    col |= ((uint16_t)(r & 0xF8) << 8);
    col |= ((uint16_t)(g & 0xFC) << 3);
    col |= ((uint16_t)(b & 0xF8) >> 3);
    */
    
    //Write_uint16_t( col, pbmp + AllHeaderOffset + x * 2 + bytes_per_row * y);
        ...
        uint8_t r, uint8_t g, uint8_t b)
{
    ...
    for (;;)
    {
        //BMP_565_SetPixelRGB(pbmp, x0, y0, r, g, b);
        ...
        uint8_t r, uint8_t g, uint8_t b)
{
    ...
    for(uint32_t y_addr = y_addr_start; y_addr >= y_addr_end; y_addr -= bytes_per_row)
    {
        for(uint32_t x_addr = x_addr_start; x_addr <= x_addr_end; x_addr += 2)
...
{
    uint32_t image_size = Read_uint32_t(pbmp_Src + FileHeaderSize + 0x14);
    for(uint32_t i = AllHeaderOffset; i < image_size; i++)
        *(pbmp_Dst+i) = *(pbmp_Src+i);
    //memcpy( (uint8_t*)(pbmp_Dst + AllHeaderOffset), (uint8_t*)(pbmp_Src + AllHeaderOffset), image_size);
}
